...
import { faqsQuery, servicesQuery } from "@/sanity/lib/queries";
import type { Faq, Service } from "@/types/sanity";
...
// Fallback data shown until CMS is populated
const fallbackServices: Service[] = [
  ...
  { _id: "2", title: "Airport pickup & drop-off", icon: "", description: "Land tired. Skip the Uber dance. We meet you at OR Tambo and have you at the rig in 35 minutes.", order: 2 },
  { _id: "3", title: "Monthly vehicle checks", icon: "", description: "Battery turned over, tyres rotated, fluids eyeballed. We send you the photo so you sleep on the long-haul.", order: 3 },
  { _id: "4", title: "Wash & detail before pickup", icon: "", description: "Show up to a clean rig. Not the dust-bowl you parked nine months ago.", order: 4 },
  { _id: "5", title: "Licensing & paperwork", icon: "", description: "Carnet expiring? Licence due? Customs query? We handle the South African admin so you don't lose a trip to bureaucracy.", order: 5 },
  { _id: "6", title: "Service & repairs", icon: "", description: "On-site fixes and trusted mechanics in our network. We project-manage; you keep planning the next leg.", order: 6 },
];
...
const fallbackFaqs: Faq[] = [
  ...
  { _id: "4", question: "Can you collect my vehicle from the port / shipping agent?", answer: "Yes. We work with most of the major shipping agents in Durban, Cape Town and Walvis Bay. We handle the collection, the paperwork, and the drive back to the facility.", order: 4 },
...
const testimonials = [
  { quote: "We left our Defender for 11 months between two trips. Came back to a vehicle that started first turn, full tank, washed, with the licence already renewed. We were on the N1 north before lunch.", name: "Marie & Pieter", country: "Belgium", vehicle: "Land Rover Defender 110", since: "2023" },
  { quote: "Antoine sent me a photo of my Hilux every month. By month four I stopped worrying. By month seven I started planning the next trip from my couch in Sydney.", name: "James", country: "Australia", vehicle: "Toyota Hilux", since: "2024" },
  { quote: "I had a customs problem I didn't even know existed. The team caught it, fixed it, and emailed me the paperwork. I was 9,000 km away. That's not storage. That's a partner.", name: "Tom", country: "UK", vehicle: "Iveco Daily 4x4", since: "2022" },
];
...
export default async function HomePage() {
  const [cmsServices, cmsFaqs] = await Promise.all([
  ...
  return (
    <>
      {/* ── Hero text (above image, AVTL-style) ── */}
      <section className="bg-parchment px-4 pb-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          {/* Eyebrow */}
          <p className="text-sm font-semibold uppercase tracking-widest text-cinnamon mb-6">
            Vehicle storage + concierge for overlanders in Johannesburg
          </p>
          {/* Two-column: H1 left / support + CTAs right */}
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-end">
            <h1 className="font-heading text-5xl font-bold text-evergreen leading-tight sm:text-6xl lg:text-7xl">
              Park your rig.<br />
              Catch your flight.<br />
              Come back ready<br />
              <span className="text-cinnamon">to roll.</span>
            </h1>
            <div className="lg:pb-3">
              <p className="text-lg text-evergreen/70 leading-relaxed max-w-md">
        ...
        <div className="mx-auto max-w-7xl grid grid-cols-2 gap-6 sm:grid-cols-4">
          {[
            ...
            <div key={item.stat}>
              ...
              <p className="text-sm text-khaki mt-1">{item.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-parchment py-24 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <Link href="/contact" className="mt-6 inline-block bg-cinnamon text-parchment px-8 py-3 rounded-lg font-semibold hover:bg-opacity-90 transition-colors">
            Build my package →
          </Link>
        </div>
      </section>
      ...
      <section className="bg-pine-teal text-parchment py-24 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <Link href="/contact" className="mt-12 inline-block bg-cinnamon text-parchment px-8 py-3 rounded-lg font-semibold hover:bg-opacity-90 transition-colors">
      ...
      <section className="bg-evergreen text-parchment py-24 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-3">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-pine-teal rounded-xl p-6 flex flex-col gap-4">
